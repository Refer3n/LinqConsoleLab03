...
public sealed class LinqExercises
{
    ...
    public IEnumerable<string> Task04_FirstAnalyticsCourse()
    {
        return UniversityData.Courses
            ...
            .Take(1)
            .DefaultIfEmpty("No Courses for Analytics");
    }
    ...
    {
        yield return UniversityData.Enrollments.Any(c => !c.IsActive) ? "Yes" : "No";
    }
    ...
    {
        yield return UniversityData.Lecturers.All(l => !string.IsNullOrEmpty(l.Department)) ?  "Yes" : "No";
    }
    ...
    {
        yield return UniversityData.Enrollments.Count(c => c.IsActive).ToString();
    }
    ...
    public IEnumerable<string> Task11_JoinStudentsWithEnrollments()
    {
        return UniversityData.Enrollments
            .Join(UniversityData.Students,
                e => e.StudentId,
                s => s.Id,
                (e, s) => $"{s.FirstName} {s.LastName}, {e.EnrollmentDate}");
    }
    ...
    public IEnumerable<string> Task14_AverageGradePerCourse()
    {
        return  UniversityData.Enrollments
            ...
            .GroupBy(x => x.Title)
            .Select(group => $"{group.Key}, {group.Average(g => g.FinalGrade)}");
    }
    ...
    {
        return UniversityData.Lecturers.GroupJoin(
            UniversityData.Courses, 
            l => l.Id, 
            c => c.LecturerId,
    ...
    public IEnumerable<string> Task16_HighestGradePerStudent()
    {
        return UniversityData.Enrollments.Where(e => e.FinalGrade.HasValue)
            .Join(UniversityData.Students, e => e.StudentId, c => c.Id,
                (e, s) => new { s.FirstName, s.LastName, e.FinalGrade })
            .GroupBy(x => new { x.FirstName, x.LastName })
            .Select(group => $"{group.Key.FirstName} {group.Key.LastName} {group.Max(g => g.FinalGrade)}");
    }
    ...
    public IEnumerable<string> Challenge01_StudentsWithMoreThanOneActiveCourse()
    {
        return UniversityData.Enrollments.Where(e => e.IsActive)
            ...
            .Select(group => $"{group.Key.FirstName} {group.Key.LastName}, {group.Count()}");
    }
    ...
    {
        return UniversityData.Enrollments
            .Join(UniversityData.Courses,  e => e.CourseId, c => c.Id,
                (e, c) => new { c.Title, c.StartDate, e.FinalGrade })
            .Where(x => x.StartDate is { Month: 4, Year: 2026 })
            .GroupBy(x => x.Title)
            .Where(group => group.All(x => !x.FinalGrade.HasValue))
    ...
    public IEnumerable<string> Challenge03_LecturersAndAverageGradeAcrossTheirCourses()
    {
        var result = UniversityData.Lecturers
            .Select(l => new
            {
                l.FirstName,
                l.LastName,
                AvgGrade = UniversityData.Courses
                    .Where(c => c.LecturerId == l.Id)
                    .SelectMany(c => UniversityData.Enrollments.Where(e => e.CourseId == c.Id))
                    .Where(e => e.FinalGrade.HasValue)
                    .Select(e => e.FinalGrade)
                    .Average()
            });
        return result.Select(r => $"{r.FirstName} {r.LastName}," +
                                  $" Average Grade: {r.AvgGrade?.ToString("F2") ?? "N/A"}");
    }
    ...
    {
        var result = UniversityData.Students.Join(
        ...
        return result.Select(r => $"{r.City}: {r.Count}");
    }
